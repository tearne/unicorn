use psutil::memory::{VirtualMemory, os::linux::VirtualMemoryExt};
use rgb::RGB8;
...
pub fn colour_array(num_px: usize) -> Vec<RGB8> {
    ...
    let result: Vec<RGB8> = (0..num_px).map(|idx|{
        let px_percent_limit = (1.0 + idx as f32) / num_px as f32;
        if px_percent_limit < used_percent { GREEN } 
        else if px_percent_limit < used_percent + buffers_percent { BLUE }
        else { BLACK }
    }).collect();
