...
impl Cpu {
    pub fn new<T: Display>(display: &T) -> Self {
        ...
        let quarters = vec![
            rect_ids(0..half_width, 0..half_height),
            rect_ids(half_width..dims.width, 0..half_height),
            rect_ids(0..half_width, half_height..dims.height),
            rect_ids(half_width..dims.width, half_height..dims.height),
        ];
    ...
    pub fn get_px(&mut self) -> Vec<bool> {
        ...
        let loads: Vec<f64> = self
            ...
            .unwrap()
            .iter()
            .map(|cpu| ((100.0 - cpu.idle()) / 100.0) as f64)
            .collect();
